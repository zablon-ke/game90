...
function bosa_charity_fundraiser_default_styles(){
	...
	if( get_theme_mod( 'footer_layout', 'footer_two' ) == 'footer_one' || get_theme_mod( 'footer_layout', 'footer_two' ) == 'footer_two' || get_theme_mod( 'footer_layout', 'footer_two' ) == 'footer_four' ){
	...
	if( get_theme_mod( 'skin_select', 'default' ) == 'dark' ){
		$css .= '
			...
			.header-four .header-icons .header-contact a {
				color: #B1B1B1;
			}
			...
			.section-popular-area .popular-items {
			    box-shadow: 0px 0px 15px 0px rgb(221 221 221 / 35%);
			}
	...
	# Responsive Teams
	if( get_theme_mod( 'disable_mobile_teams', false ) ){
		$css .= '
			@media screen and (max-width: 767px){
				.section-teams-area {
	    			display: none;
...
add_action( 'wp_head', 'bosa_charity_fundraiser_default_styles', 99 );